...
(
	set -ex
	$SCP create-coreos-docker-store update-coreos apply-user-data root@$IP:/usr/sbin/
	$SCP cloud-config.yml root@$IP:/root
	$SCP ../coreos/user-data root@$IP:/etc/user-data.yml
...
echo
echo "  tugboat create deis1 -i $IMAGE_ID -r $REGION -p true -k $SSH_ID -s $SIZE"